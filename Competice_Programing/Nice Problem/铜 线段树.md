...
记 `pre[x]` 数组为 `max(a[i]-ac,0)+max(d[i]-dr)<=x` 的数量。钦定当前答案为 `k` ,则有 `pre[k]=k` ,即 `pre[k]-k==0`，且`k`前缀的每一个位置 `i` 都有 `pre[i]>0` ,即`pre[i]-i>0`。则答案 `k` 为第一个 `pre[i]-i=0` 的位置 `i`,即 `pre[i]-i` 序列第一个前缀最小值`<=0`的位置`i` 。线段树维护 `pre[i]-i` 序列，利用区间最小值，线段树上二分找第一个`pre[i]-i==0` 的位置即为答案
```cpp
