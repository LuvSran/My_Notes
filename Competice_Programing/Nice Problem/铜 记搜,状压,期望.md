...
## Key
五个骰子，每个骰子6面，三回合。考虑记搜。`dp[S][k]`表示当前为第`k`回合，确定的骰子面为`S`状态下用最佳策略继续投的期望。`S`为十进制，`eg.1121`表确定了四个骰子分别为第1,1,2,1面。由`S`位数长度确定还要投几个骰子，六进制枚举所有投掷情况。对于一种投掷情况，二进制枚举固定哪几个，并取期望最大。所有投掷情况最大期望求和除以总情况数即为`dp[S][k]`.
```cpp
